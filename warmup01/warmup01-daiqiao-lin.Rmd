...
![_A representation of Kazuda Xiono_](https://vignette.wikia.nocookie.net/starwars/images/a/a3/Kazuda.jpg/revision/latest?cb=20180830051228)
...
![_A representation of baked buffalo wings_](https://farm5.staticflickr.com/4877/46906309351_613c8400a5_z.jpg)

Blue cheese are **encouraged** to dress the food
here are the ingrediet of the blue cheese
...
In Cartesian coordinates, if $\mathbf{p} = (p1, p2,\ldots, pn)$ and $\mathbf{q} = (q1, q2,\ldots, qn)$ are two points in Euclidean n-space, then the distance (\mathbf{d}) from \mathbf{p} to \mathbf{q}, or from \mathbf{q} to \mathbf{p} is given by the Pythagorean formula:
$$d(\mathbf{p},\mathbf{q})=d(\mathbf{p},\mathbf{q})=\sqrt{(q_1-p_1)^2+(q_2-p_2)^2+\cdots+(q_n-p_n)^2}$$ $$=\sum_{i=1}^n \sqrt{(q_i-p_i)^2}$$.
